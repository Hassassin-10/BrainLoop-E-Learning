...
export default function AudioNavigationControl() {
  ...
  }, [router, studentId, firebaseUser, toast]);
      ...
      if (SpeechRecognition) {
        ...
        recognitionRef.current.onerror = (event) => {
          ...
          if (event.error === 'no-speech') {
            ...
            errorMsg = 'A microphone problem occurred. Please check your permissions and hardware.';
          } else if (event.error === 'not-allowed') {
            errorMsg = 'Microphone access has been denied. Please enable it in your browser settings.';
  ...
  }, [userSettings, processCommand, toast]);
  ...
  useEffect(() => {
    ...
    getAudioNavigationSettings(firebaseUser.uid).then(settings => {
      const newSettings = settings || { isEnabled: true, preferredLanguage: 'en-US' };
      setUserSettings(newSettings);
      if (!settings) {
        saveAudioNavigationSettings(firebaseUser.uid, newSettings); // Save defaults if none exist
      }
    });
  }, [firebaseUser, isAuthenticated]);
  
  useEffect(() => {
    // Initialize or re-initialize whenever settings change
  ...
  const handleToggleListen = () => {
    ...
    if (isListening) {
      ...
      try {
        ...
        // This catch block handles errors if .start() itself fails (e.g., already running)
        if (e.name === 'InvalidStateError') {
             toast({ title: "Mic Error", description: "Microphone is already active. Please wait.", variant: "destructive"});
        } else {
